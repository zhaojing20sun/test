...
use Com\Utils\Prpcrypt;
use Com\Utils\SHA1;
use Com\Utils\HttpCurl;
/*use Vendor\Weixin\Utils\Prpcrypt;
use Vendor\Weixin\Utils\SHA1;*/
...
class Wechat
{
    ...
    private function checkSignature() 
    {
        if ($this->signature !== false && $this->timestamp !== false && $this->nonce !== false) {
            ...
            if ($tmp_signature === false) {
                //writelog("log.txt","Validate signature");
    ...
    private function getMessage()
    {
     ...
     //自己写的获得消息
    public function my_getMessage()
    {
        $postStr = $GLOBALS["HTTP_RAW_POST_DATA"];
        if(!empty($postStr))
        {
         $postObj = simplexml_load_string($postStr, 'SimpleXMLElement', LIBXML_NOCDATA);
         $this->message =$postObj;
         return $postObj;
        }else
        {
            return false;
    ...
    {
        $url = self::API_DOMAIN . 'cgi-bin/menu/create?access_token=' . $this->access_token;
    ...
    {
        $url = self::API_DOMAIN . 'cgi-bin/menu/delete?access_token=' . $this->access_token;
