...
<html lang="zh-CN">
  <head>
   ...
   <meta content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" name="viewport">
    ...
    <!-- Bootstrap --><link href="/new_lgx/Public/common/css/webiaoqing.css" rel="stylesheet" type="text/css">
    ...
    <link rel="stylesheet" type="text/css" href="http://at.alicdn.com/t/font_at4nh6q3wp39dx6r.css">
     <link rel="stylesheet" type="text/css" href="http://at.alicdn.com/t/font_svvd1deqtn4b1emi.css">
    <link href="/new_lgx/Public/weixin/new_detial/css/index.css" rel="stylesheet">
    ...
    <div class="container-fluid" style="margin-bottom:50px">
      <div class="row">
        <div class="col-xs-12 col-md-12">
          ...
          <div class="release-time1" style="width:100%;margin-top:20px;">
...
<span style="position:relative;top:-35px;color:window;float:right;background-color:red;border: 1px solid red;padding: 2px 10px;border-radius:3px;" class="guanzhu" title="<?php echo ($news["wid"]); ?>"><?php $re=M('weixin_attention')->where(array('wid'=>$_SESSION['wid'],'aid'=>$news['wid']))->find(); if($re){ echo "已关注"; }else{ echo "关 注"; } ?></span>
      ...
      <?php } ?>
        ...
        <!--   <div class="detail-video">
          ...
          <div class="detail-article" style="margin-top:10px;">
           <?php $img=M('weixin_release_pic')->where(array('aid'=>$news['id']))->select(); if($img){ ?>
            <div class="swiper-container">
        <div class="swiper-wrapper">
           <?php foreach ($img as $key => $value) { ?>
            <div class="swiper-slide" style="height:90%">
            <img src="/new_lgx<?php echo $value['showpic'] ?>" title="/new_lgx<?php echo ($value["pic"]); ?>" style="max-width:100%;max-height:100%;" class="showpic">
        ...
        <!-- Add Pagination -->
        <?php $count2=count($img); if($count2>1){ ?>
        <div class="swiper-pagination" style="bottom:30px"></div>
        <?php }?>
    </div>
    <?php } ?>
    <div style="text-indent:1em;margin-bottom:10px;">
           <?php echo (htmlspecialchars_decode($news["content"])); ?>
          </div>
<span style="font-size:13px;color:gray"><i class="icon iconfont icon-liulan" style="font-size:18px"></i>  浏览  <?php
 if($news['view']>10000){ echo round($news['view']/10000,1).'w'; }else{ echo $news['view']; } ?> 人</span>
                  
                       <!--   <span style="position:relative;left:52%">   <i class="icon iconfont icon-dianzan" style="font-size:18px;"></i> <?php
 if($like_man>10000){ echo round($like_man/10000,1).'w'; }else{ echo $like_man; } ?>人</span> -->
                    
                       <i class="icon iconfont icon-jiantouxia" id="bianji" onClick="tobianji()" style="font-size:18px;color:#ccc;float:right;margin-right:3%"></i>
            
      <div class="am-share am-modal-active" style="display:none" id="bianji1">
    <!--     --><?php if($news['wid']==$_SESSION['wid']){ ?>
  <a href="<?php echo U('new_write',array('nid'=>$news['id']));?>"><h5 class="am-share-title">编辑</h5></a>
  <a href="<?php echo U('release_delete',array('id'=>$news['id']));?>" onclick="return confirm('确定要删除吗')"><h5 class="am-share-title">删除</h5></a>
<?php } ?>
  <h5 class="am-share-title jubao" >举报</h5>
  
 
  <div class="am-share-footer"><button class="share_btn" style="font-size:13px;">取消</button></div>
</div>
 ...
 <img src='<?php echo M('weixin_man')->where(array('id'=>$v['wid']))->getField('headimgurl') ?>'  onerror="this.src='/new_lgx/Public/common/images/nopic.gif'"    style='width:50px;border-radius:50%;display:inline-block'><span style='margin-left:5px;'><?php echo M('weixin_mydata')->where(array('wid'=>$v['wid']))->getField('nickname')?M('weixin_mydata')->where(array('wid'=>$v['wid']))->getField('nickname'):M('weixin_man')->where(array('id'=>$v['wid']))->getField('nickname') ?></span><br>
<div class="message" style="background:lightgrey;min-height:70px;width:80%;margin-left:14%;border-radius:8px;padding:2%">
  <p style="color:gray;font-size:13px;"> <?php $wida=M('weixin_release_message')->where(array('id'=>$v['rwid']))->find(); echo M('weixin_mydata')->where(array('wid'=>$wida['wid']))->getField('nickname')?M('weixin_mydata')->where(array('wid'=>$wida['wid']))->getField('nickname'):M('weixin_man')->where(array('id'=>$wida['wid']))->getField('nickname') ?> 发表于 <?php echo date('Y-m-d H:i:s',$wida['addtime']) ?></p>
<?php echo htmlspecialchars_decode($wida['content']) ?>
</div>
...
<?php } ?>
  ...
  <nav class=" navbar-default navbar-fixed-bottom navbar-bottom-custom navbar-bottom-height" style="height:45px;padding:2% 10px">
     ...
     <textarea type="text" id="content"  style="display:none" >
     </textarea>
<article id="form_article" contenteditable="true" onkeydown="myInput.listen(this, event);" style="width:49%;height:26px;border:1px solid #ccc;border-radius:8px;padding:0 2%;display:inline-block;position:relative;top:-2px;"></article>

<div class="page_emotion box_swipe" id="page_emotion" style="width:100%;overflow:hidden;position:absolute;left:-1px;top:70px;">
  ...
  <i class="icon iconfont icon-biaoqing" style="font-size:20px;color:#ccc;margin:0;position:relative;top:2px;" id="biaoqing" title="1"></i>
...
<input type="hidden" value="<?php echo $_SESSION['wid'] ?>" id="wid">
  ...
  <script src="/new_lgx/Public/common/js/jquery-1.8.3.min.js" ></script>
     <script src="/new_lgx/Public/common/js/webiaoqin.js" ></script>
     <script type="text/javascript">
      var timeoutevent=0;
    $(function(){
...
/*  $("#bianji").click(function(){
  ...
  $(".sharebg-active").click(function(){
   ...
   $(".detail-article").click(function(){
     $("#page_emotion").css({'top':'70px'})
   })
    $("dd").click(function(){
     $("#page_emotion").css({'top':'70px'})
   })
    ...
    $("#form_article").click(function(){
      ...
      $("#biaoqing").click(function(){
        ...
        if(title==1){
           ...
           $("#page_emotion").css({'top':'-165px','background':'white'})
 ...
 $("#xin1").click(function(){
  ...
  $(".jubao").click(function(){
     location.href="<?php echo U('new_release_jubao');?>?rid="+aid
     /* var jubao=prompt("请填写举报理由","")

       $.ajax({
          url:"<?php echo U('release_jubao');?>",
          type:'post',
          data:"rid="+aid+"&wid="+wid+"&content="+jubao,
          success:function(msg){
              if(msg=='1'){
                  alert('举报成功')
                  location.reload()
