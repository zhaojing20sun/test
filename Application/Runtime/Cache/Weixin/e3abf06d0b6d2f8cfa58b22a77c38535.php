      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-xs-12 col-md-12 icon-container">
            ...
            <i style="margin-right:80px">电视直播</i>
          </div>
        </div>
      </div>
    </nav>
    ...
    <!--滑动导航条-->
      <div class="swiper-container" style="position:fixed;">
       <div class="swiper-wrapper" <?php  $count=count($column); if($numb+4>=$count){ $num=($count-3)*102.75; echo "style='transition-duration: 0ms; transform: translate3d(-".$num."px, 0px, 0px);'"; }elseif($numb=='') { echo "style='transition-duration: 0ms; transform: translate3d(0px, 0px, 0px);'"; }else{ $num=($numb+1)*102.75; echo "style='transition-duration: 0ms; transform: translate3d(-".$num."px, 0px, 0px);'"; } ?>>
            <div class="swiper-slide <?php if($_GET['vid']=='') echo 'active_li'?>"><a href="<?php echo U('show_list');?>" style="font-size:13px;">推荐</a></div>
              <?php if(is_array($column)): foreach($column as $k=>$v): ?><div   class="swiper-slide <?php if($_GET['vid']==$v['id']) echo 'active_li'?>"><a href="<?php echo U('show_list',array('vid'=>$v['id'],'num'=>$k));?>" style="font-size:13px;"><?php echo ($v["title"]); ?></a></div><?php endforeach; endif; ?>
