...
<a href="<?php echo U('article_add');?>" class="btn btn-primary" style="   margin-top: 10px;">
            添加文章</a> -->
             <span style="color:red">动态详情页地址 http://<?php echo $_SERVER['SERVER_NAME']?>/weixin/index/new_release/id/<?php echo $_SESSION['uid']?>/nid/动态ID.html</span>
            </div>

<table class="table table-bordered table-hover" style="text-align:center;    margin-top: 10px;">
                            <thead>
                             <tr><th>举报ID</th><th>内容</th><th style="width:150px;">配图</th><th>举报人</th><th>举报类型</th><th>举报内容</th><th>举报时间</th><th>操作</th></tr>
                            </thead>
                            <tbody>
                         ...
                         <td style="vertical-align:middle;max-width:150px;">
                                <?php echo ($v["rid"]); ?></td>
                                <td style="vertical-align:middle;max-width:150px;">
                                <?php $content=M('weixin_release')->where(array('id'=>$v['rid']))->getField('content'); ?>
                                <span title="<?php echo ($content); ?>" ><?php echo (htmlspecialchars_decode($content)); ?></span></td>
                           <td style="vertical-align:middle;">
                             <?php $yaowen=M('weixin_release_pic')->where(array('aid'=>$v['rid']))->select(); if($yaowen){ ?>  
                            ...
                            <td style="vertical-align:middle;"><?php echo M('weixin_man')->where(array('id'=>$v['wid']))->getField('nickname')?M('weixin_man')->where(array('id'=>$v['wid']))->getField('nickname'):M('weixin_mydata')->where(array('wid'=>$v['wid']))->getField('nickname') ?></td>
                           <td style="vertical-align:middle;">
                             <?php switch ($v['type']) { case '1': echo '色情低俗'; break; case '2': echo '政治敏感'; break; case '3': echo '违法暴力'; break; case '4': echo '暴恐血腥'; break; }?>
                           </td>
                            <td style="vertical-align:middle;"><?php echo ($v["content"]); ?></td>
                             <td style="vertical-align:middle;"><?php echo (date('Y-m-d H:i:s',$v["addtime"])); ?></td>
                            <td style="vertical-align:middle;"> 
                           
                                
                                  <a href="<?php echo U('jubao_delete',array('id'=>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
