...
<a href="<?php echo U('video_column');?>" class="btn btn-primary" style="   margin-top: 10px;">
            分类管理</a>
<a href="<?php echo U('video_add');?>" class="btn btn-primary" style="   margin-top: 10px;">
            添加视频信息  </a>&nbsp;
            <span style="color:red"> 视频详情页地址 http://<?php echo $_SERVER['SERVER_NAME']?>/weixin/index/new_video_end/id/<?php echo $_SESSION['uid']?>/nid/视频ID.html</span>

            </div>
<table class="table table-bordered table-hover" style="text-align:center;    margin-top: 10px;">
                            <thead>
                             <tr><th>视频ID</th><th>标题</th><th>类别</th><th>直播地址</th><th>图片</th><th>排序</th><th>操作</th></tr>
                            </thead>
                            <tbody>
                            <?php if(is_array($com)): foreach($com as $key=>$v): ?><tr>
                         <td style="vertical-align:"><?php echo ($v["id"]); ?></td>
                                <td style="vertical-align:middle;width:200px;"><?php echo ($v["title"]); ?></td>
                                <td style="vertical-align:middle;width:80px;"><?php echo M('weixin_video_column')->where(array('id'=>$v['pid']))->getfield('title') ?></td>
                           ...
                           <td style="vertical-align:middle;"><img src="/new_lgx/<?php echo ($v["logo"]); ?>" width="50"></td>
                            ...
                            <td style="vertical-align:middle;width:160px;"> 
                                 <a href="<?php echo U('video_change',array('id'=>$v['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('video_delete',array('id'=>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
...
$(function(){
  $(".order").change(function(){
       ...
       $.ajax({
          url:"<?php echo U('video_ordera_change');?>",
          type:'post',
