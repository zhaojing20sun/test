...
<a href="<?php echo U('video_column_add');?>" class="btn btn-primary" style="   margin-top: 10px;">
            添加分类</a>
            <span style="color:red">分类字数尽量在2-3个字之间</span>
            </div>
...
<td style="vertical-align:middle;"><input type="text" class="order" value="<?php echo ($v["ordera"]); ?>" style="text-align:center;border:none;width:60px;"><input type="hidden" value="<?php echo ($v["id"]); ?>"></td>
                            <td style="vertical-align:middle;"> 
                                 <a href="<?php echo U('video_column_change',array('id'=>$v['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('video_column_delete',array('id'=>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
...
$(function(){
  $(".order").change(function(){
       ...
       $.ajax({
          url:"<?php echo U('video_order_change');?>",
          type:'post',
