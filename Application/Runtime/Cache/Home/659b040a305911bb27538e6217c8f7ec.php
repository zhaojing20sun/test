...
<div id="right">
   <form role="form" action="<?php echo U('baoliao_change_save');?>" name="adminadd"  method="post" enctype="multipart/form-data">
                               ...
                               <input type="hidden" name="id" value="<?php echo ($lanmu["id"]); ?>" />
                                    <div class="form-group">
                                        <label><i style="color:red;"></i> 标题：</label>
                                        <div class="input-group" style="margin-top:10px;margin-left:15px;">
                                         <input name="title" id="realname" type="text" class="form-control input-xlarge" required oninvalid="setCustomValidity('标题不能为空')" oninput="setCustomValidity('')"  value="<?php echo ($lanmu["title"]); ?>"/>
                                        </div>
                                    </div>
                                     <div class="form-group" style="margin-top:10px;">
                                        <label><i style="color:red;"></i> 类别：</label>
                                        <div class="input-group" style="margin-top:10px;margin-left:15px;">
                                        <select name="type">
                                           <option value="0">请选择类别</option>
                                          <?php if(is_array($column)): foreach($column as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if($lanmu['type']==$v['id']) echo 'selected'?>><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
                                        </select>
                                        </div>
                                    </div>
                                 
                                     <div class="form-group" style="margin-top:10px;">
                                        <label><i style="color:red;"></i> 当前封面图片：</label>
                                        <div class="input-group" style="margin-top:10px;margin-left:15px;">
                                      <?php if(!$pic){ ?>
                                       <img src="/new_lgx/<?php echo ($lanmu["path"]); ?>" onerror="this.src='/new_lgx/Public/common/images/nopic.gif'" style="width:150px;height:100px;margin:0 2px">
                                       <?php }else{ ?>
                                      <?php if(is_array($pic)): foreach($pic as $key=>$v1): ?><img src="/new_lgx/<?php echo ($v1["pic"]); ?>" onerror="this.src='/new_lgx/Public/common/images/nopic.gif'" style="width:150px;height:100px;margin:0 2px"><?php endforeach; endif; ?>
                                      <?php } ?>
                                        </div>
                                    </div>
                                        <div class="form-group" style="margin-top:10px;">
                                        <label><i style="color:red;"></i> 封面图片：</label><span style="color:red">如有请选择一张或3张图片作为封面</span>
                                        <div class="input-group" style="margin-top:10px;margin-left:15px;">
                                         <input name="pic[]" id="pic" type="file" class="form-control input-xlarge"  value=""/>

                                         <br> <br>
                                         <span style="color:green" id="more">再来一张</span>
                                        </div>
                                    </div>
                                      <div class="form-group" style="margin-top:10px;">
                                        <label><i style="color:red;"></i> 内容：</label> 
                                        <div class="input-group" style="margin-top:10px;">
                                              ...
                                              <textarea rows="30" cols="80" placeholder="" name="content" id="editor_id"><?php echo ($lanmu["content"]); ?></textarea>
                                              </div>
                                              </div>
                                        </div>
                                </div>
                                <div class="form-actions" style="padding-left:25%">
 ...
 $(function(){
  $("#more").click(function(){
     ...
     if(size<3){
       $("#pic").after("<br><input type='file' name='pic[]' style='margin-top:10px'>")
     }
