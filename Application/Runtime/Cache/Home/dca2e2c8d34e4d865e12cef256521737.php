...
<a href="<?php echo U('set_guanjian_add');?>" class="btn btn-primary">
            添加关键词</a><br><br>
       <table class="table table-bordered table-hover" style="text-align:center;">
                            <thead >
                             <tr ><th style="vertical-align:middle;text-align:center">关键词</th><th style="vertical-align:middle;text-align:center">回复消息类型</th><th style="vertical-align:middle;text-align:center">是否禁用</th><th style="vertical-align:middle;text-align:center">操作</th></tr>
                            </thead>
                            <tbody>
                            <?php if(is_array($keyword)): foreach($keyword as $key=>$v): ?><tr>
                            
                               <td style="vertical-align:middle;" ><?php echo ($v["title"]); ?></td>
                              
                                 <td style="vertical-align:middle;width:300px;"><?php if($v['style']==0){echo '文本消息';}else{ echo '图文消息';} ?></td>
                                 
                                 <td  style="vertical-align:middle;"><?php if($v['passed']==0){echo '是';}else{ echo '否';} ?></td>
                                <td style="vertical-align:middle;"> 
                                 <a href="<?php echo U('set_guanjian_passed',array('id'=>$v['id'],'status'=>$v['passed']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i><?php if($v['passed']==0){echo '启用';}else{ echo '禁用';} ?></a>
                                 <a href="<?php echo U('set_guanjian_add',array('id' =>$v['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('set_guanjian_alldelete',array('id' =>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
