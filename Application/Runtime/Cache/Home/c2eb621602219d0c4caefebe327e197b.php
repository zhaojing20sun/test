...
<a href="<?php echo U('word_add');?>" class="btn btn-primary" style="   margin-top: 10px;">
            添加敏感词</a>
        
            </div>
            <form action="<?php echo U('word');?>" method="get"> <input type="text" placeholder="敏感词搜索" name="title" value="<?php echo ($_GET['title']); ?>" >    <button type="submit" class="btn1 green  btn">   搜索   </button>  </form>
            <br> 
            <form action="<?php echo U('upload_word');?>" method="post"  enctype="multipart/form-data">
            批量导入 <input type="file" name="guanjian" required style="width:200px"> <span style="color:red">(仅支持txt，关键词请用空格隔开)</span>
            <a href="<?php echo U('word_down');?>">查看示例</a>
             <button type="submit" class="btn1 green  btn" style="width:50px;height:24px;">   上传   </button>
          </form>
...
<table class="table table-bordered table-hover" style="text-align:center;    margin-top: 10px;">
                            ...
                            <?php if(is_array($com)): foreach($com as $key=>$v): ?><tr>
                                ...
                                <td style="vertical-align:middle;"><?php echo ($v["word"]); ?></td>

                            <td style="vertical-align:middle;"> 
                                 <a href="<?php echo U('word_add',array('id'=>$v['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('word_delete',array('id'=>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
