               ...
               <dt class="menu">微信管理</dt>
                ...
                <dl>
                  ...
                  <dt class="menu">信息管理</dt>
                    ...
                    <dd>
                            <a href="<?php echo U('article');?>" <?php if($active == 'article'): ?>class="active"<?php else: ?> class="normal"<?php endif; ?>><span class="ico101">文章管理</span></a>
                        </dd>
                           <dd>
...
<a href="<?php echo U('product_column_add');?>" class="btn btn-primary" style="   margin-top: 10px;">
            添加分类</a>
            </div>
<table class="table table-bordered table-hover" style="text-align:center;    margin-top: 10px;">
                            <thead>
                             <tr><th>标题</th><th>图标</th><th>操作</th></tr>
                            </thead>
                            <tbody>
                            <?php if(is_array($com)): foreach($com as $key=>$v): ?><tr>
                         
                                <td style="vertical-align:middle;"><?php echo ($v["title"]); ?></td>
                                 <td style="vertical-align:middle;"><img src="/new_lgx/<?php echo ($v["logo"]); ?>" width="50" onerror="this.src='/new_lgx/Public/common/images/nopic.gif'"></td>
                            <td style="vertical-align:middle;"> 
                                 <a href="<?php echo U('product_column_change',array('id'=>$v['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('product_column_delete',array('id'=>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>

                                </td>
                            </tr>    
                            <?php $re=M("weixin_product_column")->where(array('pid'=>$v['id']))->select(); if($re){ foreach($re as $key=>$value){ ?>      
                               <tr>
                         
                                <td style="vertical-align:middle;" colspan="2">----<?php echo ($value["title"]); ?></td>

                            <td style="vertical-align:middle;"> 
                                 <a href="<?php echo U('product_column_change',array('id'=>$value['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('product_column_delete',array('id'=>$value['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
