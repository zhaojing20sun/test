...
<a href="<?php echo U('product_add');?>" class="btn btn-primary" style="   margin-top: 10px;">
            添加商品  </a>
                 &nbsp;
            <span style="color:red"> 商品页地址 http://<?php echo $_SERVER['SERVER_NAME']?>/weixin/index/product/id/<?php echo $_SESSION['uid']?>.html</span>
            </div>
<table class="table table-bordered table-hover" style="text-align:center;    margin-top: 10px;">
                            <thead>
                             <tr><th>标题</th><th>类别</th><th>价格</th><th>图片</th><th>操作</th></tr>
                            </thead>
                            <tbody>
                            <?php if(is_array($com)): foreach($com as $key=>$v): ?><tr>
                         
                                <td style="vertical-align:middle;"><?php echo ($v["title"]); ?></td>
                                <td style="vertical-align:middle;"><?php echo M('weixin_product_column')->where(array('id'=>$v['pid']))->getfield('title') ?></td>
                           <td style="vertical-align:middle;"><?php echo ($v["price"]); ?></td>
                           <td style="vertical-align:middle;"><img src="/new_lgx/<?php echo ($v["logo"]); ?>" width="100"></td>
                            <td style="vertical-align:middle;"> 
                                 <a href="<?php echo U('product_change',array('id'=>$v['id']));?>" class="btn btn-warning  btn-sm"><i class="fa fa-pencil"></i> 修改</a>
                                  <a href="<?php echo U('product_delete',array('id'=>$v['id']));?>" class="btn btn-danger  btn-sm" onclick="return confirm('您确定要删除该项吗，如不确定请点取消')"><i class="glyphicon glyphicon-remove"></i> 删除</a>
