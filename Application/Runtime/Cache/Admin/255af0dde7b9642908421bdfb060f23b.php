                 ...
                 <?php if(is_array($sencodcolumnlist)): foreach($sencodcolumnlist as $key=>$v): ?><li class="start  <?php if($open==$v['_id']) echo 'active open'; ?>" <?php if($admin10['permission'] && $v['title']=='代理商管理') echo 'style="display:none"' ?> >
                    ...
                    <ul class="sub-menu">
                      <?php  $column = new \Think\Model\MongoModel('column'); $subcolumnlist=$column->where(array('pid' =>$v['_id']))->order('sort_order desc')->select(); foreach ($subcolumnlist as $value){ ?>
                        <li <?php if($active==$value['_id']) echo 'class="active"'; ?> <?php if($admin10['permission'] && $value['title']=='代理商列表' || $value['title']=='推广会员' || $value['title']=='提现记录' ) echo 'style="display:none"' ?>  >
                        ...
                        <div class="portlet-body form">
                            <form role="form" action="<?php echo U('adminsave');?>" name="adminadd"  method="post">
                                <div class="form-body">
                                 <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <label>登录用户名</label>
                                        <div class="input-group">
                                         <input type="text" id="username" class="form-control" placeholder="登录用户名" name="username" >
                                  ...
                                  <span  class="power"><input type="checkbox" name="power[0]" value="1">  代理商信息管理</span> 
                                  <span  class="power"><input type="checkbox" name="power[1]" value="1">  钻石管理</span>  
                                  <span  class="power"><input type="checkbox" name="power[2]" value="1">  活动管理</span>  
                                  <span  class="power"><input type="checkbox" name="power[3]" value="1">  分销管理</span>  
                                  <span  class="power"><input type="checkbox" name="power[4]" value="1">  帮助管理</span>   
                                ...
                                <div class="form-actions">
                                    <button type="submit" class="btn red">   添   加   </button>
    ...
    $(function(){
        $("#username").change(function(){
           var username=$(this).val()
            $.ajax({
                 url:"<?php echo U('check_adminusername');?>",
                 type:"post",
                 data:"username="+username,
                 success:function(msg){
                  if(msg=='1'){
                     $("#username").after("<span class='error cunzai'>该用户名已存在</span>")
                  }else{
                     $(".cunzai").remove()
                  }
                 }
